    ...
    <div class="container">
        <div class="d-flex justify-content-start align-items-center m-4">
            <div class="fw-bold text-capitalize">
                ...
                / <a href="{{ route('admin.menu_items', [$menu->id, $menu_item->id]) }}">{{ $menu_item->title }}</a>
            </div>
        </div>
        <div class="card shadow-sm m-4">
            <div class="card-header">
                @if ($menu_item->children->count() > 0)
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                                @foreach ($menu_item->children as $key => $menu_i)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $menu_i->title }}
                                        </td>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                @can('sua-menu')
                                                    <a href="{{ route('admin.edit-menu-item-children', [$menu->id, $menu_item->id, $menu_i->id]) }}"
                                                        class="btn btn-outline-primary btn-xs me-2" title="Edit"><i
                                                            class="fas fa-edit" data-bs-toggle="tooltip"
                                                            title="Chỉnh sửa menu"></i></a>
                                                @endcan
                                                @can('xoa-menu')
                                                    <form action="{{ route('admin.menu_item.destroy', $menu_i->id) }}"
                ...
                @else
                    <p class="alert alert-danger">Chưa có menu con nào!</p>
                @endif
            </div>
            <div class="card-body">
                            ...
                            @foreach ($menu->items as $menu_parent)
                                @if ($menu_parent->parent_id == null)
                                    <option value="{{ $menu_parent->id }}"
                                        {{ $menu_parent->id == $menu_item->id ? 'selected' : '' }}>
